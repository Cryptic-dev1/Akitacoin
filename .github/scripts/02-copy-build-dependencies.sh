...
if [[ ${GITHUB_REF} =~ "release" || ${FORCEBUILDDEPS} = "1" ]]; then
    ...
    cd /tmp
    curl -O https://akitacoin-build-resources.s3.amazonaws.com/${OS}/akitacoin-${OS}-dependencies.tar.gz
    curl -O https://akitacoin-build-resources.s3.amazonaws.com/${OS}/SHASUM
    if [[ $(sha256sum -c /tmp/SHASUM) ]]; then
        cd ${GITHUB_WORKSPACE}/depends
        tar zxvf /tmp/akitacoin-${OS}-dependencies.tar.gz
    else
