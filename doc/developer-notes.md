...
Run with the -testnet option to run with "play akitas" on the test network, if you
...
- AkitacoinMiner : Generates akitas (if wallet is enabled).
...
be sent upstream but bugfixes may also be prudent to PR against Akitacoin so that they can be integrated
