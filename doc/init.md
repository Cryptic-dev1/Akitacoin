Sample init scripts and service configuration for akitacoind
==========================================================
...
systemd. Directories are given a permission of 710, giving the akitacoin group
access to files under it _if_ the files themselves give permission to the
akitacoin group to do so (e.g. when `-sysperms` is specified). This does not allow
for the listing of files under the directory.

NOTE: It is not currently possible to override `datadir` in
`/etc/akitacoin/akitacoin.conf` with the current systemd, OpenRC, and Upstart init
files out-of-the-box. This is because the command line options specified in the
init files take precedence over the configurations in
`/etc/akitacoin/akitacoin.conf`. However, some init systems have their own
configuration mechanisms that would allow for overriding the command line
options specified in the init files (e.g. setting `AKITACOIND_DATADIR` for
OpenRC).
