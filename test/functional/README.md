...
- Use a module-level docstring to describe what the test is testing, and how it
  is testing it.
- When subclassing the AkitacoinTestFramwork, place overrides for the
...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to control whether
  ...
  has 25 mature blocks (25x5000=125000 AKIC) in its wallet.
...
with the akitacoind(s) being tested (using python's asyncore package); the other
implements the test logic.

- `NodeConn` is the class used to connect to a akitacoind.  If you implement
