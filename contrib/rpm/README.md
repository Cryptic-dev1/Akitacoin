...
The RPM spec file provided here is for Akitacoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://akiccrypto.com/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Akitacoin git repository](https://github.com/JustAResearcher/Akitacoin) but are not part of
...
Currently only `Source10` is used, for the example `akitacoin.conf` file.
...
in the akitacoin git repository. At present only an SVG version of the Akitacoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the akitacoin
...
needed when 0.12.1 is released, a proper fix is already in the Akitacoin git
master branch.
...
idea to build Akitacoin Core against a static version of that library compiled
...
The `akitacoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `akitacoin-server` package. At this point, most RPM based
...
The `akitacoin-server` package should have SELinux support. How to properly do
...
Akitacoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/akitacoin/). He has been
packaging Akitacoin for Fedora at least since 2012.
...
4. I renamed the `akitacoin` package that contains the Qt GUI to `akitacoin-core` as
that appears to be how the general population refers to it, in contrast to
`akitacoin-xt` or `akitacoin-classic`. I wanted to make sure the general population
