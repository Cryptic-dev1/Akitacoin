 ...
 * between akitacoind, and akitacoin-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    RenameThread("akitacoin-shutoff");
    ...
    FlushWallets();
#endif
    GenerateAkitacoins(false, 0, GetParams());
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), AKITACOIN_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/AkitacoinChain/Akitacoin>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("akitacoin-loadblk");
...
/** Sanity checks
 *  Ensure that Akitacoin is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Akitacoin process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", AKITACOIN_CONF_FILENAME)).string());
    ...
    GenerateAkitacoins(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
