...
const std::string AkitacoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString AkitacoinGUI::DEFAULT_WALLET = "~Default";
...
static bool ThreadSafeMessageBox(AkitacoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

AkitacoinGUI::AkitacoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void AkitacoinGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Akitacoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and akitacoin: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to AKIC addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Akitacoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Akitacoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a akitacoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Akitacoin command-line options").arg(tr(PACKAGE_NAME)));
...
void AkitacoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void AkitacoinGUI::createToolBars()
{
    if(walletFrame)
    {
        QSettings settings;
        ...
        if(IconsOnly) {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/akictext")));
        }
        else {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/akitacointext")));
        }
...
QString tbStyleSheet = ".QToolBar {background-color: transparent; border-color: transparent; }  "
        ...
        QLabel* akitacoinLabel = new QLabel("AKITACOIN");
        akitacoinLabel->setAlignment(Qt::AlignCenter);
        akitacoinLabel->setStyleSheet("color: white; font-weight: bold; font-size: 20px;"); // Adjust the font size as needed
        ...
        QVBoxLayout* akitaLabelLayout = new QVBoxLayout(toolbarWidget);
        akitaLabelLayout->addWidget(labelToolbar);
        akitaLabelLayout->addWidget(m_toolbar);
        akitaLabelLayout->addStretch(1);
        akitaLabelLayout->addWidget(akitacoinLabel);
        akitaLabelLayout->addWidget(versionLabel);  // Add the QLabel for version
        akitaLabelLayout->setDirection(QBoxLayout::TopToBottom);
        ...
        labelCurrentMarket->setText(tr("Akitacoin Market Price"));
        ...
        comboAkicUnit = new QComboBox(headerWidget);
        QStringList list;
        for(int unitNum = 0; unitNum < CurrencyUnits::count(); unitNum++) {
            list.append(QString(CurrencyUnits::CurrencyOptions[unitNum].Header));
        }
        comboAkicUnit->addItems(list);
        comboAkicUnit->setFixedHeight(26);
        comboAkicUnit->setContentsMargins(5,0,0,0);
        comboAkicUnit->setStyleSheet(STRING_LABEL_COLOR);
        comboAkicUnit->setFont(currentMarketFont);

        labelVersionUpdate->setText("<a href=\"https://github.com/JustAResearcher/Akitacoin/releases\">New Wallet Version Available</a>");
        ...
        priceLayout->addWidget(comboAkicUnit, 0 , Qt::AlignBottom| Qt::AlignLeft);
        ...
        connect(comboAkicUnit, SIGNAL(activated(int)), this, SLOT(currencySelectionChanged(int)));
        // Create the timer
        ...
        // Get the latest Akitacoin release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/JustAResearcher/Akitacoin/releases";
...
void AkitacoinGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/akictext")));
        m_toolbar->setMaximumWidth(65);
...
void AkitacoinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool AkitacoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool AkitacoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void AkitacoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void AkitacoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void AkitacoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void AkitacoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void AkitacoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void AkitacoinGUI::optionsClicked()
{
...
void AkitacoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void AkitacoinGUI::showDebugWindow()
{
...
void AkitacoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void AkitacoinGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void AkitacoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void AkitacoinGUI::openClicked()
{
...
void AkitacoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void AkitacoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void AkitacoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void AkitacoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void AkitacoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void AkitacoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** AKIC START */
void AkitacoinGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void AkitacoinGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void AkitacoinGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void AkitacoinGUI::gotoRestrictedAssetsPage()
{
...
void AkitacoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Akitacoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void AkitacoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void AkitacoinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void AkitacoinGUI::updateHeadersSyncProgressLabel()
{
...
void AkitacoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void AkitacoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Akitacoin"); // default title
...
void AkitacoinGUI::changeEvent(QEvent *e)
{
...
void AkitacoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void AkitacoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void AkitacoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "AKIC")
        msg += tr("Amount: %1\n").arg(AkitacoinUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(AkitacoinUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void AkitacoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void AkitacoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool AkitacoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool AkitacoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void AkitacoinGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void AkitacoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void AkitacoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void AkitacoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void AkitacoinGUI::detectShutdown()
{
...
void AkitacoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void AkitacoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void AkitacoinGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(AkitacoinGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void AkitacoinGUI::subscribeToCoreSignals()
{
...
void AkitacoinGUI::unsubscribeFromCoreSignals()
{
...
void AkitacoinGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void AkitacoinGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<AkitacoinUnits::Unit> units = AkitacoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const AkitacoinUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(AkitacoinUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(AkitacoinUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (AkitacoinUnits::Unit u : AkitacoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(AkitacoinUnits::name(u)), this);
...
{
    setText(AkitacoinUnits::name(newUnits));
}
...
void AkitacoinGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void AkitacoinGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "AkitacoinGUI::onPriceUnitChange: " + QString::number(newIndex);
    ...
    this->comboAkicUnit->setCurrentIndex(newIndex);
    this->getPriceInfo();
}

void AkitacoinGUI::getPriceInfo()
{
    request->setUrl(QUrl(QString("https://api.xeggex.com/api/v2/ticker/AKIC_%1").arg(this->currentPriceDisplay->Ticker)));
    networkManager->get(*request);
}

#ifdef ENABLE_WALLET
void AkitacoinGUI::mnemonic()
{
...
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/JustAResearcher/Akitacoin/releases"));
