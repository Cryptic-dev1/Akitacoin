...
bool CScript::IsAssetScript(int& nType, bool& fIsOwner, int& nStartingIndex) const
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_AKIC_ASSET) { // OP_AKIC_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == AKIC_R) { // Check to see if AKIC starts at 27 ( this->size() < 105)
                if ((*this)[28] == AKIC_V)
                    if ((*this)[29] == AKIC_N)
                        index = 30;
            } else {
                if ((*this)[28] == AKIC_R) // Check to see if AKIC starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == AKIC_V)
                        if ((*this)[30] == AKIC_N)
                            index = 31;
            }
