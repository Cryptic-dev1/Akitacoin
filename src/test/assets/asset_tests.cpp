...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of AKITACOINCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("AKIC", type));
        BOOST_CHECK(!IsAssetNameValid("AKITACOIN", type));
        BOOST_CHECK(!IsAssetNameValid("AKITACOINCOIN", type));

        //- Versions of AKITACOINCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("AKITACOIN.COIN", type));
        BOOST_CHECK(IsAssetNameValid("AKITACOIN_COIN", type));
        BOOST_CHECK(IsAssetNameValid("AKICSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERAKIC", type));
        BOOST_CHECK(IsAssetNameValid("AKITACOINSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEAKITACOIN", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_AKITACOINS", type));
        BOOST_CHECK(IsAssetNameValid("SEAKICOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("AKITACOIN", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("AKITACOIN", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
