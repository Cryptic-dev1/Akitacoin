...
UniValue UpdateAddressTag(const JSONRPCRequest &request, const int8_t &flag)
{
    ...
    if (!IsValidDestination(destination)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Akitacoin address: ") + address);
    }
    ...
    if (request.params.size() > 2) {
        ...
        if (!change_address.empty()) {
           ...
           if (!IsValidDestination(change_dest)) {
               throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Akitacoin change address: ") + change_address);
           }
        }
    }
...
UniValue issue(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 1 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "4. \"change_address\"        (string), optional, default=\"\"), address the the akic change will be sent to, if it is empty, change address will be generated for you\n"
            ...
            "8. \"ipfs_hash\"             (string, optional but required if has_ipfs = 1), an ipfs hash or a txid hash once AIP5 is activated\n"
        ...
        if (!change_address.empty()) {
            ...
            if (!IsValidDestination(destination)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                                   std::string("Invalid Change Address: Invalid Akitacoin address: ") + change_address);
            }
        }
    }
...
UniValue issueunique(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
                ...
                "Five (5) AKIC will be burned for each asset created.\n"
                ...
                "5. \"change_address\"        (string, optional, default=\"\"), address the the akic change will be sent to, if it is empty, change address will be generated for you\n"
    ...
    if (!changeAddress.empty()) {
        ...
        if (!IsValidDestination(destination)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                               std::string("Invalid Change Address: Invalid Akitacoin address: ") + changeAddress);
        }
    }
...
UniValue listassetbalancesbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"                  (string, required) a akitacoin address\n"
...
UniValue transfer(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 3 || request.params.size() > 7)
        throw std::runtime_error(
                ...
                "4. \"message\"                  (string, optional) Once AIP5 is voted in ipfs hash or txid hash to send along with the transfer\n"
                "5. \"expire_time\"              (numeric, optional) UTC timestamp of when the message expires\n"
                "6. \"change_address\"       (string, optional, default = \"\") the transactions AKIC change will be sent to this address\n"
    ...
    if (!IsValidDestination(to_dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Akitacoin address: ") + to_address);
    }
    ...
    if (!message.empty() || expireTime > 0) {
        if (!AreMessagesDeployed()) {
            throw JSONRPCError(RPC_INVALID_PARAMS, std::string("Unable to send messages until Messaging AIP5 is enabled"));
        }
    }
    ...
    std::string akic_change_address = "";
    if (request.params.size() > 5) {
        akic_change_address = request.params[5].get_str();
    }
    ...
    CTxDestination akic_change_dest = DecodeDestination(akic_change_address);
    if (!akic_change_address.empty() && !IsValidDestination(akic_change_dest))
        throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("AKIC change address must be a valid address. Invalid address: ") + akic_change_address);
    ...
    CCoinControl ctrl;
    ctrl.destChange = akic_change_dest;
...
UniValue transferfromaddresses(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 4 || request.params.size() > 8)
        throw std::runtime_error(
            "transferfromaddresses \"asset_name\" [\"from_addresses\"] qty \"to_address\" \"message\" expire_time \"akic_change_address\" \"asset_change_address\"\n"
            + AssetActivationWarning() +
            ...
            "5. \"message\"                  (string, optional) Once AIP5 is voted in ipfs hash or txid hash to send along with the transfer\n"
            "6. \"expire_time\"              (numeric, optional) UTC timestamp of when the message expires\n"
            "7. \"akic_change_address\"       (string, optional, default = \"\") the transactions AKIC change will be sent to this address\n"
    ...
    std::string akic_change_address = "";
    if (request.params.size() > 6) {
        akic_change_address = request.params[6].get_str();
    }
...
UniValue transferfromaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 4 || request.params.size() > 8)
        throw std::runtime_error(
                "transferfromaddress \"asset_name\" \"from_address\" qty \"to_address\" \"message\" expire_time \"akic_change_address\" \"asset_change_address\"\n"
                + AssetActivationWarning() +
                ...
                "5. \"message\"                  (string, optional) Once AIP5 is voted in ipfs hash or txid hash to send along with the transfer\n"
                "6. \"expire_time\"              (numeric, optional) UTC timestamp of when the message expires\n"
                "7. \"akic_change_address\"       (string, optional, default = \"\") the transaction AKIC change will be sent to this address\n"
...
UniValue reissue(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() > 7 || request.params.size() < 3)
        throw std::runtime_error(
                ...
                "7. \"new_ipfs\"                 (string, optional, default=\"\"), whether to update the current ipfs hash or txid once AIP5 is active\n"
...
UniValue listtagsforaddress(const JSONRPCRequest &request)
{
    ...
    CTxDestination dest = DecodeDestination(address);
    if (!IsValidDestination(dest))
        throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("Not valid AKIC address: ") + address);
...
UniValue checkaddresstag(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedAssetsDeployed() || request.params.size() != 2)
        throw std::runtime_error(
                ...
                "\nArguments:\n"
                "1. \"address\"          (string, required) the AKIC address to search\n"
...
UniValue issuequalifierasset(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 1 || request.params.size() > 6)
        throw std::runtime_error(
                ...
                "6. \"ipfs_hash\"             (string, optional but required if has_ipfs = 1), an ipfs hash or a txid hash once AIP5 is activated\n"
...
UniValue issuerestrictedasset(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedAssetsDeployed() || request.params.size() < 4 || request.params.size() > 9)
        throw std::runtime_error(
                ...
                "5. \"change_address\"        (string, optional, default=\"\") address that the akic change will be sent to, if it is empty, change address will be generated for you\n"
                ...
                "9. \"ipfs_hash\"             (string, optional but required if has_ipfs = 1) an ipfs hash or a txid hash once AIP5 is activated\n"
...
UniValue reissuerestrictedasset(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedAssetsDeployed() || request.params.size() < 3 || request.params.size() > 9)
        throw std::runtime_error(
                ...
                "6. \"change_address\"        (string, optional, default=\"\") address that the akic change will be sent to, if it is empty, change address will be generated for you\n"
                ...
                "9. \"new_ipfs\"              (string, optional, default=\"\") whether to update the current ipfs hash or txid once AIP5 is active\n"
...
UniValue transferqualifier(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() > 3) {
        ...
        CTxDestination change_dest = DecodeDestination(change_address);
        if (!IsValidDestination(change_dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Akitacoin address: ") + change_address);
        }
    }
...
#ifdef ENABLE_WALLET
    { "assets",   "transferfromaddress",        &transferfromaddress,        {"asset_name", "from_address", "qty", "to_address", "message", "expire_time", "akic_change_address", "asset_change_address"}},
    { "assets",   "transferfromaddresses",      &transferfromaddresses,      {"asset_name", "from_addresses", "qty", "to_address", "message", "expire_time", "akic_change_address", "asset_change_address"}},
